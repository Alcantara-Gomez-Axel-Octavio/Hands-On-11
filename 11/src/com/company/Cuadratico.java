...
public class Cuadratico {
    ...
    private static Individuo cruce(Individuo padre1, Individuo padre2) {
        ...
        return new Individuo(nuevoB0, nuevoB1, nuevoB2, 10);
    }
    ...
    public static double calcularFitnessCuadratico(List<DataSet> datos, double b0, double b1, double b2) {
        double SStotal = 0;
        double SSerror = 0;
        double x;
        double y;
        ...
        // Calcular la suma total de los cuadrados (SStotal)
        for (DataSet p : datos) {
            y = p.getY();
            SStotal += Math.pow(y - yPromedio, 2);
        }

        // Calcular la suma de los cuadrados de los residuos (SSerror)
        for (DataSet p : datos) {
            x = p.getX();
            y = p.getY();
            double residuo = y - (b0 + b1 * x + b2 * x * x);
            SSerror += residuo * residuo;
        }

        // Calcular el coeficiente de determinación R2
        double R2 = 1 - (SSerror / SStotal);
    ...
    public static ArrayList<Individuo> generarPoblacionInicial(List<DataSet> datos) {
        ...
        double rangoB0Min = 30.0;
        double rangoB0Max = 31.0;

        double rangoB1Min = 0.9;
        double rangoB1Max = 1.0;

        double rangoB2Min = 0.003;
        double rangoB2Max = 0.010;
        ...
        for (int i = 0; i < POBLACION; i++) {
            ...
            poblacionInicial.add(new Individuo(b0, b1, b2, 10 ));
        }
